...
/** Renders a Material Receipt Note (goods received) to a shareable PDF and returns a content:// Uri. */
object MaterialReceiptPdf {

    private const val PAGE_W = 420
    private const val MARGIN = 24f

    fun generate(context: Context, company: CompanyInfo, receipt: MaterialReceipt, lines: List<MaterialReceiptItem>): Uri {
        val doc = PdfDocument()
        val lineHeight = 20f
        ...
        y += 4f
        c.drawText("MATERIAL RECEIPT NOTE", MARGIN, y, h); y += 18f

        c.drawText("Receipt No: ${receipt.receiptNo}", MARGIN, y, p)
        c.drawText("Date: ${Format.date(receipt.dateMillis)}", right, y, rightP); y += 16f
        c.drawText("Supplier: ${receipt.supplierName}", MARGIN, y, p)
        if (receipt.lpoNo.isNotBlank()) c.drawText("vs LPO: ${receipt.lpoNo}", right, y, rightP)
        y += 14f
        ...
        if (receipt.remarks.isNotBlank()) {
            y += 6f
            c.drawText("Remarks: ${clip(receipt.remarks, 50)}", MARGIN, y, p)
        }

        doc.finishPage(page)

        val dir = File(context.cacheDir, "shared").apply { mkdirs() }
        val safe = receipt.receiptNo.ifBlank { "material_receipt" }.replace(Regex("[^A-Za-z0-9_-]"), "_")
