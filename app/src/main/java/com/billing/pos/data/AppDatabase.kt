...
abstract class AppDatabase : RoomDatabase() {
    ...
    companion object {
        ...
        /** Recurring journals (Accounting Batch 2): templates that generate a real journal
         * voucher each time they come due (rent, EMI, ...). */
        private val MIGRATION_79_80 = object : androidx.room.migration.Migration(79, 80) {
            override fun migrate(db: androidx.sqlite.db.SupportSQLiteDatabase) {
                db.execSQL(
                    "CREATE TABLE IF NOT EXISTS `recurring_journals` (" +
                        "`id` INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, " +
                        "`name` TEXT NOT NULL, `narration` TEXT NOT NULL, `frequency` TEXT NOT NULL, " +
                        "`startDate` INTEGER NOT NULL, `nextDueDate` INTEGER NOT NULL, `endDate` INTEGER NOT NULL, " +
                        "`active` INTEGER NOT NULL, `lastGeneratedAt` INTEGER NOT NULL, `occurrencesGenerated` INTEGER NOT NULL)"
                )
                db.execSQL(
                    "CREATE TABLE IF NOT EXISTS `recurring_journal_lines` (" +
                        "`id` INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, " +
                        "`templateId` INTEGER NOT NULL, `headId` INTEGER NOT NULL, `headName` TEXT NOT NULL, " +
                        "`amount` REAL NOT NULL, `isDebit` INTEGER NOT NULL)"
                )
            }
        }

        /** Depreciation (Accounting Batch 2): tracks the last date each fixed asset was
         * depreciated up to, so re-running depreciation only covers the elapsed period since. */
        private val MIGRATION_80_81 = object : androidx.room.migration.Migration(80, 81) {
            override fun migrate(db: androidx.sqlite.db.SupportSQLiteDatabase) {
                db.execSQL("ALTER TABLE fixed_assets ADD COLUMN lastDepreciatedMillis INTEGER NOT NULL DEFAULT 0")
            }
        }
        ...
        /** The customer's own app now reports whether they're premium for this shop on every
         *  outgoing chat message (see CustomerMessageSend/ShopMessagesFetch) — stored here so the
         *  shop owner's own 100-messages/24h chat cap (see ShopMessagesViewModel.send) can exempt
         *  a premium customer. Existing rows default false, which is correct: they predate the
         *  cap entirely, and the next incoming message from that customer will refresh it. */
        private val MIGRATION_104_105 = object : androidx.room.migration.Migration(104, 105) {
            override fun migrate(db: androidx.sqlite.db.SupportSQLiteDatabase) {
                db.execSQL("ALTER TABLE shop_messages ADD COLUMN customerPremium INTEGER NOT NULL DEFAULT 0")
            }
        }
